...
module.exports = async function (context, req) {
    ...
    try {
        ...
        // simple query
        const result = await connection.query('UPDATE `masters` SET (userId=?, countId=?, janVal=?, febVal=?, marVal=?, aprVal=?, mayVal=?, junVal=?, julVal=?, agoVal=?, sepVal=?, octVal=?, novVal=?, decVal=?, year=?, initBalance=?, finalBalance=?) WHERE id=?', [userId, countId, janVal, febVal, marVal, aprVal, mayVal, junVal, julVal, agoVal, sepVal, octVal, novVal, decVal, year, initBalance, finalBalance, masterId]);
        console.log(result);
